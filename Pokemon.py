...
def change_pokemon(window, offset, values, pokemon_data):
    ...
    pokemon_data = pokemon_func(str(pokemon_id), values['graphic'], values['shiny'], values['who'])
...
layout = [[sg.Text('Enter Pokemon Name or ID:'), sg.InputText(key="pokemon name")],
          ...
          [sg.Button('Previous', disabled=True), sg.Text(key='pokemon data'), sg.Button('Next', disabled=True)],
...
while True:
    ...
    if event == 'OK' or event == 'pokemon name' + '_Enter':
        ...
        pokemon_data = pokemon_func(values['pokemon name'], values['graphic'], values['shiny'], values['who'])
